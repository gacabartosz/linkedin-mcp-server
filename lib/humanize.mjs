...
import { stripAiMarks } from './ai-marks.mjs';
...
export function humanizeText(input, opts = {}) {
  ...
  // 1) Slady generatora: znaki niewidoczne, nietypowe spacje, fancy typografia.
  //    Tablice siedza w lib/ai-marks.mjs (port skilla remove-ai-marks) — tu tylko
  //    wywolanie, zeby JEDNO miejsce definiowalo, co jest sladem AI.
  //    Uwaga: myslnikow ai-marks NIE rusza, robi to punkt 3 nizej (lepsze reguly:
  //    zakres 10–20 zostaje liczba, punktor na poczatku linii zostaje "- ").
  t = stripAiMarks(t).text;
  t = t.replace(/[‹›]/g, "'"); // ‹ › → ' (katowe pojedyncze, poza mapa skilla)
